...
Man kontrolliert die Wahrscheinlichkeit eines \textbf{Fehlers 1. Art} mit $\alpha$. Man wählt also $K$ bzw. $\alpha$ (je nach dem was gegeben ist) wie folgt:
\begin{equation*}
...
Um die Wahrscheinlichkeit eines \textbf{Fehlers 2. Art} zu vermindern, muss man die Funktion $\beta$ möglichst gross wählen:
\begin{equation*}
...
\subsection{z-Test, Test für Erwartungswerte bei bekannter Varianz}
...
\begin{split}
	...
	\textbf{Verwerfungsbereich:}\quad & (-\infty, -c_{\not =}) \cup (c_{\not =}, \infty) \qquad\text{für } H_A: \vartheta \neq \vartheta_0 \text{ (beidseitig)} \\ 
	& (-\infty, c_<) \qquad\text{für } H_A: \vartheta < \vartheta_0 \text{ (einseitig)} \\ 
	& (c_>, \infty) \qquad\text{für } H_A: \vartheta > \vartheta_0 \text{ (einseitig)} \\
...
\subsection{t-Test, Test für Erwartungswerte bei unbekannter Varianz}

\begin{equation*}
\begin{split}
	\textbf{Annahme:}\quad & E[X_i] = \mu,\ \Var[X_i] = S^2 = \frac{1}{n-1}\sum_{i =1}^n (X_i - \overline{X}_n)^2 \text{ (abgeschätzt) } \\
	...
	\textbf{Verwerfungsbereich:}\quad & (-\infty, -c_{\not =}) \cup (c_{\not =}, \infty) \qquad\text{für } H_A: \mu \neq \mu_0 \text{ (beidseitig)} \\ 
	& (-\infty, c_<) \qquad\text{für } H_A: \mu < \mu_0 \text{ (einseitig)} \\ 
	& (c_>, \infty) \qquad\text{für } H_A: \mu > \mu_0 \text{ (einseitig)} \\
...
\emph{Bemerkung:} $z_{m, \gamma}$ bezeichnet das $\gamma$-Quantil einer $t_m$-Verteilung. Für diesen Wert gilt $P[X \leq t_{m, \gamma}] = \gamma$.
