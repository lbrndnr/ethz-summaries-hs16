...
Sei $Z = X + Y$, wobei $X$ und $Y$ unabhängig sind, also $p(x,y) = p(x)p(y)$. Dann gilt:
\begin{equation*}
	p_Z(z) = \sum_{x_k \in \mathcal{W}(X)} p_X(x_k)p_Y(z-x_k) = \sum_{y_k \in \mathcal{W}(Y)} p_Y(y_k)p_X(z-x_k)
\end{equation*}
...
\begin{split}
	...
	\textbf{Dichtefunktion:}&\qquad f_X(t) = P[X = t] = F_X'(t)
\end{split}
\end{equation*}
...
Sei $Z = XY$ mit $f_Z=f_{X,Y}$ für $-\infty < x \leq y < \infty$, dann gilt:
\begin{equation*}
	E[Z] = E[XY] = \int_{-\infty}^\infty \int_{x}^\infty x y f_{XY}(x, y)\ dx\ dy
\end{equation*}
...
Sei $Z = X + Y$, wobei $X$ und $Y$ unabhängig sind, also $p(x,y) = p(x)p(y)$. Dann gilt:
\begin{equation*}
	p_Z(z) = \int_{-\infty}^\infty f_X(x)f_Y(z-x)\ dx = \int_{-\infty}^\infty f_Y(y)f_X(z-y)\ dy
\end{equation*}
...
\subsubsection{Beispiel affin/linear: $g(x) = ax + b$}
...
\begin{split}
	...
	& S_n = \sum_{i=1}^n X_i \\
	\textbf{Ansatz:}\quad & \lim_{n\rightarrow\infty} P[\frac{S_n - E[S_n]}{\sqrt{\Var[S_n]}} \leq \frac{x - n\mu}{\sigma \sqrt{n}}] \\
	& = \lim_{n\rightarrow\infty} P[\frac{S_n - n\mu}{\sigma \sqrt{n}} \leq \frac{x - n\mu}{\sigma \sqrt{n}}] = \Phi(\frac{x - n\mu}{\sigma \sqrt{n}})
\end{split}
\end{equation*}
...
Seien $X_1,...,X_n$ unabh{\"a}ngig mit $X_i \sim Be(p_i)$ und $S_n = \sum_{i=1}^n X_i$. Sei ferner $\mu_n := E[S_n] = \sum_{i = 1}^n p_i$ und $\delta > 0$, dann gilt:
\begin{equation*}
...
\subsection{Konstruktion eines Tests}

\subsubsection{Signifikanzniveau $\alpha$}

Man kontrolliert die Wahrscheinlichkeit eines \textbf{Fehlers 1. Art} mit $\alpha$. Man w{\"a}hlt also $K$ bzw. $\alpha$ (je nach dem was gegeben ist) wie folgt:
\begin{equation*}
	P_\vartheta[T \in K] \leq \alpha
\end{equation*}

\subsubsection{Macht}

Um die Wahrscheinlichkeit eines \textbf{Fehlers 2. Art} zu vermindern, muss man die Funktion $\beta$ m{\"o}glichst gross w{\"a}hlen:
\begin{equation*}
	\beta: \Theta_A \mapsto [0, 1] \qquad \vartheta \mapsto \beta(\vartheta) := P[T \in K]
\end{equation*}
...
\section{Wichtige Tests}

\subsection{Allgemein}

\begin{equation*}
\begin{split}
	\textbf{Verwerfungsbereich:}\quad & (-\infty, -c_{\not =}) \cup (c_{\not =}, \infty) \qquad\text{f{\"u}r } H_A: \vartheta \neq \vartheta_0 \text{ (beidseitig)} \\ 
	& (-\infty, c_<) \qquad\text{f{\"u}r } H_A: \vartheta < \vartheta_0 \text{ (einseitig)} \\ 
	& (c_>, \infty) \qquad\text{f{\"u}r } H_A: \vartheta > \vartheta_0 \text{ (einseitig)} \\
	\textbf{P-Wert:}\quad & P_{H_0}[|T| > x] = 2(1- \Phi(x)) \\ 
	& P_{H_0}[T > x] = 1- \Phi(x) \\ 
	& P_{H_0}[T \leq x] = \Phi(x) \\
\end{split}
\end{equation*}

\emph{Bemerkung:} Beim P-Wert handelt es sich um das kleinste Signifikanzniveau $\alpha$, welches das Verwerfen der Nullhypothese impliziert.

\emph{Tipp:} $\Phi(-x) = 1 - \Phi(x)$

\subsection{z-Test, Test f{\"u}r Erwartungswerte bei bekannter Varianz}

\begin{equation*}
\begin{split}
	\textbf{Annahme:}\quad & X_i \sim \mathcal{N}(\vartheta, \sigma^2) \Rightarrow E[X_i] = \vartheta,\ \Var[X_i] = \sigma^2 \\
	\textbf{Hypothese:}\quad & H_0:\ \vartheta = \vartheta_0 \\
	\textbf{Teststatistik:}\quad & T := \frac{\overline{X}_n - \vartheta_0}{\sigma/ \sqrt{n}} \sim \mathcal{N}(0, 1) \\
	\textbf{Kritischer Wert:} \quad & c_{\not =} = \Phi^{-1}(1- \frac{\alpha}{2}) = z_{1-\frac{\alpha}{2}} \\
	& c_< = - \Phi^{-1}(1-\alpha) = z_\alpha = -z_{1-\alpha} \\
	& c_> = \Phi^{-1}(1-\alpha) = z_{1-\alpha}
\end{split}
\end{equation*}

\emph{Bemerkung:} $z_{1-\alpha}$ bezeichnet das $(1-\alpha)$-Quantil.

\subsection{t-Test, Test f{\"u}r Erwartungswerte bei unbekannter Varianz}

\begin{equation*}
\begin{split}
	\textbf{Annahme:}\quad & E[X_i] = \mu,\ \Var[X_i] = S^2 = \frac{1}{n-1}\sum_{i =1}^n (X_i - \overline{X}_n)^2 \text{ (abgesch{\"a}tzt) } \\
	\textbf{Hypothese:}\quad & H_0:\ \mu = \mu_0 \\
	\textbf{Teststatistik:}\quad & T := \frac{\overline{X}_n - \mu_0}{S/\sqrt{n}} \sim t_{n-1} \qquad \text{($t$-verteilt mit $n-1$ Freiheitsgraden)} \\
	\textbf{Kritischer Wert:} \quad & c_{\not =} = t_{n-1, 1-\frac{\alpha}{2}} \\
	& c_< = t_{n-1, \alpha} = -t_{n-1, 1-\alpha} \\
	& c_> = t_{n-1, 1-\alpha}
\end{split}
\end{equation*}

\emph{Bemerkung:} $z_{m, \gamma}$ bezeichnet das $\gamma$-Quantil einer $t_m$-Verteilung. F{\"u}r diesen Wert gilt $P[X \leq t_{m, \gamma}] = \gamma$.

\section{Wichtige Zweistichproben-Tests}
Bei Zweistichproben-Tests geht man aus von Zufallsvariablen $X_1,...,X_n$ und $Y_1,...,Y_m$, die unter $P_{\vartheta}$ alle unabh{\"a}ngig sind; zudem sind die $X_i$ und die $Y_j$ unter $P_{\vartheta}$ jeweils f{\"u}r sich betrachtet i.i.d.

\subsection{Gepaarter Zweistichprobentest ($m=n$)}
\begin{equation*}
\begin{split}	
		\textbf{Stichproben:}\quad & X_1,...,X_n \overset{i.i.d.}{\sim} \mathcal{N}(\mu_X, \sigma^2)\\
												  & Y_1,..., Y_n \overset{i.i.d.}{\sim} \mathcal{N}(\mu_Y, \sigma^2)\\
		\textbf{Idee:} \quad & \text{Zur{\"u}ckf{\"u}rung auf den Fall einer Stichprobe (Vergleich von $\mu_X$ und $\mu_Y$}) \\
											& Z_i := X_i  - Y_i \overset{i.i.d.}{\sim} \mathcal{N}(\mu_X - \mu_Y, 2\sigma^2) \quad \text{unter } P_\vartheta\\
		\textbf{Ansatz:} \quad & 1) \text{ Falls Varianz $\sigma^2$ bekannt } \Rightarrow \text{z-Test} \\
											& \qquad T = \frac{(\overline{X}_n - \overline{Y}_n)-(\mu_X - \mu_Y)}{\sigma \sqrt{2/n}}  \sim \mathcal{N}(0,1)  \\
											& 2) \text{ Falls Varianz $\sigma^2$ unbekannt } \Rightarrow \text{t-Test} \\		
											& \qquad T = \frac{(\overline{X}_n - \overline{Y}_n)-(\mu_X - \mu_Y)}{S \sqrt{2/n}}  \sim t_n  \\	
											& \qquad \text{wobei } \quad S^2 = \frac{1}{2}(S_X^2 + S_Y^2) = \frac{1}{2} S_Z^2 \\
											& \qquad \text{ mit } S_X^2 =\frac{1}{n-1}\sum_{i =1}^n (X_i - \overline{X}_n)^2	\qquad \text{analog f{\"u}r $S_Y^2 und S_Z^2$}												
\end{split} 
\end{equation*}



\subsection{Ungepaarter Zweistichprobentest ($m \not = n$)}
\begin{equation*}
\begin{split}	
		\textbf{Stichproben:}\quad & X_1,...,X_n \overset{i.i.d.}{\sim} \mathcal{N}(\mu_X, \sigma^2)\\
												  & Y_1,..., Y_m \overset{i.i.d.}{\sim} \mathcal{N}(\mu_Y, \sigma^2)\\
		\textbf{Problem:} \quad & \text{Man kann nicht mehr paarweise Differenzen bilden, auch wenn zuf{\"a}llig} \\
										&m=n \text{ ist.}\\
		\textbf{Ansatz:} \quad & 	1) \text{ Falls Varianz $\sigma^2$ bekannt} \Rightarrow \text{z-Test} \\	
											& \qquad 	T = \frac{(\overline{X}_n - \overline{Y}_m)-(\mu_X - \mu_Y)}{\sigma \sqrt{\frac{1}{n} +\frac{1}{m}}}  \sim \mathcal{N}(0,1)  \\	
											& 2) \text{ Falls Varianz $\sigma^2$ unbekannt}	\Rightarrow \text{t-Test} \\	
											& \qquad T = \frac{(\overline{X}_n - \overline{Y}_m)-(\mu_X - \mu_Y)}{S \sqrt{\frac{1}{n} +\frac{1}{m}}}  \sim t_{n+m-2} \\	
											& \qquad 	\text{wobei } \quad S^2 = \frac{1}{m+n-2}((n-1)S_X^2 + (m-1)S_Y^2)	\\
											& \qquad \text{mit } \quad 	S_X^2 =\frac{1}{n-1}\sum_{i =1}^n (X_i - \overline{X}_n)^2	\quad \text{und}	\quad S_Y^2 =\frac{1}{m-1}\sum_{i =1}^m (Y_i - \overline{Y}_m)^2					  
\end{split} 
\end{equation*}
